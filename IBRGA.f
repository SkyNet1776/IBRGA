...
C
C
C     TRY KEEPING ARRAYS TOGETHER (NOT SPLIT LINES)
C
C
...
95    FORMAT((' FOR PROPELLANT NUMBER',I2,/' IMPETUS OF PROPELLANT J/G'
     ...
     &14.6,/' DENSITY OF PROPELLANT G/CM**#',E14.6/' RATIO OF SPECIFIC 
     &HEATS FOR PROPELLANT',E14.6/' NUMBER OF PERFORATIONS OF PROPELLAN
     &T',I2/' LENGTH OF PROPELLANT GRAIN CM',E14.6/' DIAMETETER OF INNE
     &R PERFORATION IN PROPELLANT GRAINS CM',E14.6/' DIAMETER OF OUTER 
     &PERFORATION OF PROPELLANT GRAINS CM',E14.6/' OUTSIDE DIAMETER OF 
C     PAGE 17
     &PROPELLANT GRAIN CM',E14.6/' DISTANCE BETWEEN PERF CENTERS CM',E1
     &4.6)//)
      TMPI=0.0
      DO 96 I=1,NPROP
...
96    CONTINUE
     ...
     &,NBR(J))
      WRITE(3,110,ERR=30)NBR(J),(ALPHA(J,I),BETA(J,I),PRES(J,I),I=1,NBR
     &(J))
...
231   CONTINUE
C     PAGE 20
C     TENERGY=ELPT+ELPR+ELGPM+ELBR+ELRC+ELHT+ELAR
...
264   CONTINUE
      DO 21 I=1,NDE
C
C     Z(I) AND P(I) EXPERIENCE NAN ERROR
C
      D(I)=(Z(I)-B(J)*P(I))*A(J)
      Y(I)=DELTAT*D(I)+Y(I)
...
327   FORMAT(1X,'VELOCITY OF PROJECTILE M/S ',E14.6,' AT THIS TIME MSEC
     &',E14.6)
...
10    MASSF=0.
      ...
      IF(NP.EQ.0)GO TO 2000
      IF(NP.EQ.1)GO TO 3000
      IF(LM2X.GT.0)GO TO 340
C     PAGE 24
      SURF=0.
      V=0.
      GO TO 850
340   S0=PI*LM2X*(D+P1+6.*P+12.*X)+HAFPI*(DM2X*DM2X-P1P2X*P1P2X-6.
     &*PP2X*PP2X)
...
390   Z=(2.*D1+P+P1+4.*X)/4.
      ...
      F2=A3/4.*P12XSQ+A4/4.*PP2XSQ-SQRT(Z*(Z-D1)*(2.*Z-P-TWOX)*(2.*Z-P1
     &-TWOX))
...
C
3000  IF(D-P-4.*X.LE.0.0) GO TO 3001
      TWOX=X+X
